...
# Tier 1 — API E2E: every CRUD surface + full RAG flow (59 tests, ~40s)
docker compose exec backend python -m pytest tests_e2e/ -v

# Tier 2 — browser E2E with Playwright (16 tests, ~40s)
cd frontend
npx playwright install chromium   # first time only
npm run e2e
```

Tier 1 covers auth, users/documents/sessions/feedback/sharing CRUD with ownership checks, semantic cache behavior, signed media streaming (Range/tampering/expiry), the admin panel APIs, service-status banners (publish/schedule/update/end, the simulated monitor, the keyed external-monitor webhook), and rate limiting. Tier 2 drives the real UI in Chromium: login/guest, streamed answers with citations, the source panel and signed document opening, sidebar rename/delete/search/collapse, public share links, admin upload/delete and user management, service banners (outage on the login page pausing the guest chat, dismissible notices, the `/admin/avisos` flow), theme persistence, and the mobile overlay.

To demo an outage from a terminal the way an external monitor would report it: `.\scripts\simulate-incident.ps1 -Open -EtaMinutes 45 -Contact "45454545"`, then `-Resolve`.
